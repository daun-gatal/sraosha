...
from sraosha.services.contract_team import resolve_team_id_from_doc
...
async def test_resolve_team_id_invalid_uuid_string() -> None:
    async with TestSessionLocal() as db:
        tid, errs = await resolve_team_id_from_doc(db, {"team_id": "not-a-uuid"}, {})
        assert tid is None
        assert errs
...
async def test_resolve_team_id_unknown_uuid() -> None:
    async with TestSessionLocal() as db:
        missing = uuid.uuid4()
        tid, errs = await resolve_team_id_from_doc(db, {"team_id": str(missing)}, {})
        assert tid is None
        assert errs
...
async def test_resolve_team_id_by_uuid_ok() -> None:
    ...
    async with TestSessionLocal() as db:
        ...
        await db.commit()
    async with TestSessionLocal() as db:
        tid, errs = await resolve_team_id_from_doc(db, {"team_id": str(uid)}, {})
        assert tid == uid
        assert not errs
...
async def test_resolve_team_id_by_owner_name_existing() -> None:
    ...
    async with TestSessionLocal() as db:
        ...
        await db.commit()
    async with TestSessionLocal() as db:
        tid, errs = await resolve_team_id_from_doc(db, {"owner_team": "alpha"}, {})
        assert tid == uid
        assert not errs
...
async def test_resolve_team_id_by_owner_name_missing_team_no_error() -> None:
    async with TestSessionLocal() as db:
        tid, errs = await resolve_team_id_from_doc(db, {"owner_team": "ghost"}, {})
        assert tid is None
        assert not errs
